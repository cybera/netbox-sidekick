...
def generate_map():
    ...
    owner_name = settings.PLUGINS_CONFIG['netbox_sidekick'].get('mapping_primary_owner', None)
    if owner_name is None:
        return

    grenml_manager = grenml.GRENMLManager(owner_name)

    primary_site_name = settings.PLUGINS_CONFIG['netbox_sidekick'].get('mapping_primary_site', None)
