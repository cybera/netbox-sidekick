...
class MemberBandwidthServicesDataView(PermissionRequiredMixin, View):
    permission_required = 'sidekick.view_memberbandwidth'
    model = NetworkService
    ...
    def get(self, request, pk):
        ...
        (services, accounting_sources) = get_services(member)
        period = get_period(request)
        graph_data = utils.get_graphite_service_data(graphite_render_host, services, period)
        return JsonResponse({
            'graph_data': graph_data,
        })


class MemberBandwidthAccountingDataView(PermissionRequiredMixin, View):
    permission_required = 'sidekick.view_memberbandwidth'
    model = NetworkService
    ...
    def get(self, request, pk):
        ...
        (services, accounting_sources) = get_services(member)
        period = get_period(request)
        graph_data = utils.get_graphite_accounting_data(graphite_render_host, accounting_sources, period)
        return JsonResponse({
            'graph_data': graph_data,
        })


class MemberBandwidthRemainingDataView(PermissionRequiredMixin, View):
    permission_required = 'sidekick.view_memberbandwidth'
    model = NetworkService
    ...
    def get(self, request, pk):
        ...
        (services, accounting_sources) = get_services(member)
        period = get_period(request)
        graph_data = utils.get_graphite_remaining_data(graphite_render_host, services, period)
        return JsonResponse({
            'graph_data': graph_data,
        })
